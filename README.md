...
| <a name="input_internal_dns_domain"></a> [internal\_dns\_domain](#input\_internal\_dns\_domain) | Sets the root domain for the private Cloud DNS zone used for non-public, internal testing records. A value is required<br/>and must be a valid DNS domain; default value is 'f5-ai-agent-gateway.test'. | `string` | `"f5-ai-agent-gateway.test"` | no |
